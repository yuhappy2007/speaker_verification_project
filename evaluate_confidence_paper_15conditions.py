"""
Confidence模型完整测试 - 基于官方verification pairs
...
def load_confidence_model(checkpoint_path, device):
    """加载Confidence模型"""
    print(f'📦 Loading Confidence model from: {checkpoint_path}')
    ...
    print(f"   - Model type: Confidence (Dynamic Weighting)")
...
def evaluate_single_condition(mlp, speaker_model, enhancer, pairs_loader,
    ...
    # 分别记录same和diff的权重
    weights_same = {'noisy': [], 'enhanced': []}
    weights_diff = {'noisy': [], 'enhanced': []}
    ...
    for label, path1, path2 in tqdm(pairs, desc=desc, leave=False):
        ...
        # 记录平均权重
        avg_w_noisy = (w1_noisy + w2_noisy) / 2
        avg_w_enh = (w1_enh + w2_enh) / 2

        if label == 1:  # Same speaker
            weights_same['noisy'].append(avg_w_noisy)
            weights_same['enhanced'].append(avg_w_enh)
        else:  # Different speaker
            weights_diff['noisy'].append(avg_w_noisy)
            weights_diff['enhanced'].append(avg_w_enh)
    ...
    weight_analysis = {
        'same_speaker': {
            'noisy': safe_stats(weights_same['noisy']),
            'enhanced': safe_stats(weights_same['enhanced'])
        },
        'diff_speaker': {
            'noisy': safe_stats(weights_diff['noisy']),
            'enhanced': safe_stats(weights_diff['enhanced'])
        }
    }

    return eer, weight_analysis


def print_single_condition_result(noise_type, snr, eer, weight_analysis, baseline):
    """打印单个条件的结果（含权重信息）"""
    ...
    print(f"   Confidence (Ours):   {eer:6.2f}%  (Δ {improvement:+.2f}%, {improvement_pct:+.1f}%)")

    # 打印权重分析
    w_same = weight_analysis['same_speaker']
    w_diff = weight_analysis['diff_speaker']

    print(f"\n   📊 Dynamic Weights Analysis:")
    print(f"   Same Speaker  - Noisy: {w_same['noisy']['mean']:.3f} | Enhanced: {w_same['enhanced']['mean']:.3f}")
    print(f"   Diff Speaker  - Noisy: {w_diff['noisy']['mean']:.3f} | Enhanced: {w_diff['enhanced']['mean']:.3f}")
...
def generate_comparison_table(results, output_dir):
    ...
    # Markdown表格
    md_path = output_dir / 'comparison_table_confidence.md'
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write('# Confidence Model Complete Evaluation Results\n\n')
        ...
        for noise_type in ['noise', 'music', 'speech']:
            ...
            f.write(
                '| SNR | Noisy (Paper) | Enhanced (Paper) | Triplet (Paper) | Confidence (Ours) | vs Triplet | Improvement | Avg Weight (N/E) |\n')
            f.write(
                '|-----|---------------|------------------|-----------------|-------------------|------------|-------------|------------------|\n')
            ...
            for snr in [0, -5, -10, -15, -20]:
                ...
                # 获取平均权重
                w_same = result['weights']['same_speaker']
                avg_w_noisy = w_same['noisy']['mean']
                avg_w_enh = w_same['enhanced']['mean']

                f.write(f'| {snr:3d} | {baseline["Noisy"]:6.2f}% | {baseline["Enhanced"]:6.2f}% | '
                        f'{baseline["Triplet"]:6.2f}% | {eer:6.2f}% {status} | {diff:+.2f}% | '
                        f'{improv:+.1f}% | {avg_w_noisy:.2f}/{avg_w_enh:.2f} |\n')

    print(f'📄 Markdown table saved: {md_path}')

    # CSV文件（含权重）
    csv_path = output_dir / 'results_confidence.csv'
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write('Noise Type,SNR,Noisy(Paper),Enhanced(Paper),Triplet(Paper),Confidence(Ours),'
                'vs Triplet,Improvement(%),Weight_Noisy,Weight_Enhanced\n')
        ...
        for noise_type in ['noise', 'music', 'speech']:
            for snr in [0, -5, -10, -15, -20]:
                ...
                f.write(f'{noise_type},{snr},{baseline["Noisy"]:.2f},{baseline["Enhanced"]:.2f},'
                        ...
                        f'{avg_w_noisy:.3f},{avg_w_enh:.3f}\n')

    print(f'📊 CSV file saved: {csv_path}')


def generate_weight_analysis_report(results, output_dir):
    """生成权重分析报告"""
    output_dir = Path(output_dir)

    report_path = output_dir / 'weight_analysis_report.txt'
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write('=' * 80 + '\n')
        f.write('CONFIDENCE WEIGHT ANALYSIS REPORT\n')
        f.write('=' * 80 + '\n\n')

        f.write('Dynamic Weighting Behavior Analysis\n')
        f.write('-' * 80 + '\n\n')

        for noise_type in ['noise', 'music', 'speech']:
            f.write(f'\n{noise_type.upper()} Conditions:\n')
            f.write('-' * 40 + '\n')

            for snr in [0, -5, -10, -15, -20]:
                result = results[noise_type][snr]
                w_same = result['weights']['same_speaker']
                w_diff = result['weights']['diff_speaker']

                f.write(f'\nSNR = {snr}dB:\n')
                f.write(f'  Same Speaker:\n')
                f.write(f'    Noisy weight:    {w_same["noisy"]["mean"]:.3f} ± {w_same["noisy"]["std"]:.3f}\n')
                f.write(f'    Enhanced weight: {w_same["enhanced"]["mean"]:.3f} ± {w_same["enhanced"]["std"]:.3f}\n')
                f.write(f'  Different Speaker:\n')
                f.write(f'    Noisy weight:    {w_diff["noisy"]["mean"]:.3f} ± {w_diff["noisy"]["std"]:.3f}\n')
                f.write(f'    Enhanced weight: {w_diff["enhanced"]["mean"]:.3f} ± {w_diff["enhanced"]["std"]:.3f}\n')

        f.write('\n' + '=' * 80 + '\n')
        f.write('Key Observations:\n')
        f.write('- Lower SNR → Higher enhanced weight (more noise reduction needed)\n')
        f.write('- Higher SNR → Higher noisy weight (less distortion from enhancement)\n')
        f.write('=' * 80 + '\n')

    print(f'📊 Weight analysis saved: {report_path}')
...
def generate_summary_statistics(results, output_dir):
    ...
    print('\n' + '=' * 80)
    print('📊 OVERALL STATISTICS - CONFIDENCE vs TRIPLET')
    print('=' * 80)
    ...
    stats_path = Path(output_dir) / 'summary_statistics_confidence.json'
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Confidence on all 15 conditions')
    ...
    parser.add_argument('--num_pairs', type=int, default=None,
                        help='Number of pairs to test (default: all pairs)')
    parser.add_argument('--output_dir', type=str, default='results_confidence_15conditions',
    ...
    print('=' * 80)
    print('🔬 CONFIDENCE COMPREHENSIVE EVALUATION: All 15 Conditions')
    print('=' * 80)
    ...
    for noise_type in ['noise', 'music', 'speech']:
        ...
        for snr in [0, -5, -10, -15, -20]:
            ...
            eer, weight_analysis = evaluate_single_condition(
            ...
            baseline = PAPER_BASELINE[noise_type][snr]
            print_single_condition_result(noise_type, snr, eer, weight_analysis, baseline)
    ...
    results_path = output_dir / 'complete_results_confidence.json'
    with open(results_path, 'w') as f:
        json.dump({
            ...
            'config': {
                ...
                'seed': args.seed,
                'model_type': 'Confidence (Dynamic Weighting)',
    ...
    generate_weight_analysis_report(results, output_dir)
    ...
    print('\n🎯 Next steps:')
    print('   1. Check comparison_table_confidence.md for detailed results')
    print('   2. Review weight_analysis_report.txt for dynamic weight behavior')
    print('   3. Compare with SupCon results in results_supcon_15conditions/')

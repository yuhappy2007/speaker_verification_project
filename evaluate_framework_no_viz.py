...
用法：
    python evaluate_framework_no_viz.py --voxceleb_dir data/voxceleb1 --musan_dir data/musan
"""
...
def main(args):
    ...
    try:
        ...
        # 加载checkpoint
        if Path(args.speaker_model_path).exists():
            checkpoint = torch.load(args.speaker_model_path, map_location=device)
            speaker_model.load_state_dict(checkpoint['model_state_dict'])
            print(f'✅ Loaded ECAPA-TDNN: {args.speaker_model_path}')
        else:
            print(f'⚠️  Speaker model checkpoint not found: {args.speaker_model_path}')
            print('   Using randomly initialized ECAPA-TDNN (results will be meaningless)')

        speaker_model.eval()

    except ImportError:
        print('⚠️  Could not import ECAPA-TDNN, using dummy model!')

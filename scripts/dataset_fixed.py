# """
# VoxCeleb1 + MUSAN 数据集 - 修复版
# 支持：
# 1. MUSAN Train/Test划分（不相交）- 使用预生成的split文件
# 2. 测试集15种条件（3噪声 × 5 SNR）
# 3. 修复：处理单文件说话人问题
# 4. ✅ 新增：同时返回clean和noisy音频用于DeepFilterNet训练
# """
# import os
# import random
# import numpy as np
...
#     - return_clean: 是否返回clean音频（用于DeepFilterNet训练），默认True
#     """
#
...
#         self.return_clean = return_clean  # ✅ 新增
#
...
#         print(f'Return clean audio: {self.return_clean}')  # ✅ 新增
#         print(f'{"=" * 70}\n')
#
...
#         """
#         修复：
#         1. 确保positive不会与anchor相同
#         2. ✅ 新增：同时返回clean和noisy音频
#         """
#         # 获取anchor
...
#                 result['anchor_clean'] = anchor_audio
#                 result['positive_clean'] = positive_audio
#                 result['negative_clean'] = negative_audio
#
#             return result
#
#         except Exception as e:
    ...
    def __getitem__(self, idx, retry_count=0):
        ...
        try:
            ...
            if retry_count >= MAX_RETRIES:
                raise RuntimeError(
                    f"Failed to load sample {idx} after {MAX_RETRIES} retries. "
                    f"Last error: {str(e)}\n"
                    f"Sample info: speaker={self.file_to_speaker.get(str(self.audio_files[idx]), 'unknown')}, "
                    f"file={self.audio_files[idx]}"
                )

            # 第一次失败时打印警告
            if retry_count == 0:
                print(f"⚠️ Warning: Failed to load sample {idx}: {str(e)[:100]}")
                print(f"   Trying next sample...")

            # 尝试加载下一个样本（带递归计数）
            return self.__getitem__((idx + 1) % len(self), retry_count + 1)


# VoxCelebTestPairs类（备用）
class VoxCelebTestPairs(Dataset):
    """
    VoxCeleb1测试集的配对数据集（备用实现）
    ...
    def __getitem__(self, idx, retry_count=0):
        ...
        try:
            ...
            if retry_count >= MAX_RETRIES:
                raise RuntimeError(
                    f"Failed to load pair {idx} after {MAX_RETRIES} retries. "
                    f"Last error: {str(e)}"
                )

            # 第一次失败时打印警告
            if retry_count == 0:
                print(f"⚠️ Warning: Failed to load pair {idx}: {str(e)[:100]}")
                print(f"   Trying next pair...")

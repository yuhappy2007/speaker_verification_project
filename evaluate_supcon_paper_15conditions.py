"""
SupCon模型完整测试 - 基于官方verification pairs
...
class VoxCelebPairsLoader:
    """加载VoxCeleb官方验证对（与Confidence代码完全相同）"""
...
def load_supcon_model(checkpoint_path, device):
    """⭐ 修改2：加载SupCon模型（不是Confidence）"""
    print(f'📦 Loading SupCon model from: {checkpoint_path}')

    mlp = RobustEmbeddingMLP(embedding_dim=192).to(device)  # ⭐ 改这里
...
def evaluate_single_condition(mlp, speaker_model, enhancer, pairs_loader,
    ...
    for label, path1, path2 in tqdm(pairs, desc=desc, leave=False):
        ...
        # ⭐ 修改：只返回embedding，不返回权重
        emb1 = extract_supcon_embedding(noisy1, speaker_model, enhancer, mlp, device)
        emb2 = extract_supcon_embedding(noisy2, speaker_model, enhancer, mlp, device)
...
def print_single_condition_result(noise_type, snr, eer, baseline):
    ...
    print(f"   SupCon (Ours):    {eer:6.2f}%  (Δ {improvement:+.2f}%, {improvement_pct:+.1f}%)")
...
def generate_comparison_table(results, output_dir):
    ...
    # Markdown表格
    md_path = output_dir / 'comparison_table_supcon.md'
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write('# SupCon Complete Evaluation Results\n\n')
        ...
        for noise_type in ['noise', 'music', 'speech']:
            ...
            f.write(
                '| SNR | Noisy (Paper) | Enhanced (Paper) | Triplet (Paper) | SupCon (Ours) | vs Triplet | Improvement |\n')
            f.write(
                '|-----|---------------|------------------|-----------------|---------------|------------|-------------|\n')
...
def generate_summary_statistics(results, output_dir):
    ...
    print('\n' + '=' * 80)
    print('📊 OVERALL STATISTICS - SUPCON vs TRIPLET')
    print('=' * 80)
    print(f'Total conditions tested:           {total_conditions}')
    print(f'Conditions where SupCon > Triplet: {better_count}/{total_conditions} ({win_rate:.1f}%)')
    ...
    stats_path = Path(output_dir) / 'summary_statistics_supcon.json'
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate SupCon on all 15 conditions')
    parser.add_argument('--checkpoint', type=str,
                        default='checkpoints_supcon_400batch/final_model_supcon_400batch.pth',
                        help='SupCon model checkpoint')
    parser.add_argument('--num_pairs', type=int, default=None,
                        help='Number of pairs to test (default: all pairs)')
    parser.add_argument('--output_dir', type=str, default='results_supcon_15conditions',
    ...
    print('=' * 80)
    print('🔬 SUPCON COMPREHENSIVE EVALUATION: All 15 Conditions')
    print('=' * 80)
    ...
    for noise_type in ['noise', 'music', 'speech']:
        ...
        for snr in [0, -5, -10, -15, -20]:
            ...
            results[noise_type][snr] = {'eer': eer}
    ...
    results_path = output_dir / 'complete_results_supcon.json'
    ...
    print('\n🎯 Next steps:')
    print('   1. Check comparison_table_supcon.md for detailed results')
    print('   2. Review summary_statistics_supcon.json for overview')
